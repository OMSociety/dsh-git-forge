...
/**
 * Strip single/double-quoted regions so commit messages / strings do not
 * trigger `git ... push` detection (e.g. commit -m "fix bare git push").
 */
...
export function classifyGitWriteCommand(command) {
  ...
  const cmd = stripShellQuotes(raw)
  // Prefer per-segment matching so path names like dsh-git-forge + later prose
  // cannot combine into a false push detection across the whole script.
  const segments = cmd.split(/[\n\r;|&]+/).map((s) => s.trim()).filter(Boolean)
  let sawPush = false
  let sawRemoteWrite = false
  let sawGhWrite = false
  for (const seg of segments) {
    // Real git invocation: start or after sudo, not "...-git-..."
    const hasGitCmd =
      /(^|[`($])\s*(?:sudo\s+)?git(?:\s|$)/.test(seg) ||
      /(?:^|\s)(?:sudo\s+)?git(?:\s|$)/.test(seg)
    if (!hasGitCmd && !/(?:^|\s)gh(?:\s|$)/.test(seg)) continue
    if (hasGitCmd && /\bpush\b/.test(seg)) sawPush = true
    if (hasGitCmd && /\bremote\b/.test(seg) && /\b(add|set-url)\b/.test(seg)) sawRemoteWrite = true
    if (/(?:^|\s)gh(?:\s|$)/.test(seg) && /\brepo\b/.test(seg) && /\bcreate\b/.test(seg)) sawGhWrite = true
    if (/(?:^|\s)gh(?:\s|$)/.test(seg) && /\bpr\b/.test(seg) && /\bcreate\b/.test(seg)) sawGhWrite = true
  }
