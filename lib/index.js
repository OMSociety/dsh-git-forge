...
async function handleApi(method, body, ctx) {
  if (method === 'health') {
    return { ok: true, version: VERSION, name: 'dsh-git-forge' }
  }
...
function registerGitForgeTool(ctx) {
  if (!defineTool || !ctx.tools) return () => {}
  ...
  const tool = defineTool({
    ...
    async execute(args, exec) {
      ...
      const projectPathKey = normalizeProjectKey(args.project_path || '')
        || resolveProjectFromExec(ctx, exec)
